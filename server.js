...
app.get('/', (req, res) => {
  ...
  const configScript = `
<script>
  window.SUPABASE_URL = "${escapeForJS(process.env.SUPABASE_URL || '')}";
  window.SUPABASE_ANON_KEY = "${escapeForJS(process.env.SUPABASE_ANON_KEY || '')}";
  window.WA_NUMBER = "${escapeForJS(process.env.WHATSAPP_BOT_NUMBER || process.env.OWNER_PHONE || '972507983306')}";
</script>`;
